...
vec3 orderedDither(vec2 fragCoord, vec3 color, float colorCount, float pixelSize) {
  vec2 scaledCoord = floor(fragCoord / pixelSize);
  int x = int(mod(scaledCoord.x, 8.0));
  int y = int(mod(scaledCoord.y, 8.0));
  float threshold = bayerMatrix8x8[y * 8 + x] - 0.5;
  float stepSize = 1.0 / max(1.0, colorCount - 1.0);

  color += threshold * stepSize;
  color = clamp(color, 0.0, 1.0);
  return floor(color * (colorCount - 1.0) + 0.5) / (colorCount - 1.0);
}
...
void main() {
  ...
  vec3 green = vec3(0.22, 0.82, 0.55);
  vec3 violet = vec3(0.42, 0.32, 0.84);

  vec3 color = mix(deep, cyan, smoothstep(0.12, 0.92, field));
  color = mix(color, green, smoothstep(0.5, 0.86, detailWave) * 0.34);
  color = mix(color, violet, smoothstep(0.68, 0.98, band) * 0.24);
